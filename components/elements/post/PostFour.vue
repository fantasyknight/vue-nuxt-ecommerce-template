<template>
	<div
		class="post post-mask gradient"
		:class="{'post-video': post.type === 'video'}"
	>
		<figure :class="`post-media ${post.type === 'gallery' ? '' : 'overlay-zoom'}`">
			<nuxt-link
		...
		<div class="post-details">
			<div class="post-meta">
				on <a
					href="javascript:;"
					class="post-date"
				>{{ date.toLocaleDateString( 'en-US', { year: 'numeric', month: 'short', day: "2-digit", timeZone: "UTC" } ) }}</a>
				| <a
					href="javascript:;"
					class="post-comment"
			...
			<nuxt-link
				:to="`/blog/single/${post.slug}`"
				class="btn btn-link btn-underline btn-white"
